...
Liveness redesigned 2026-07-31 against the observer evidence
(the private design notes): the venue sends NO snapshot, NO heartbeat and
NO sequence number (0 of 1,419 recorded messages), and a subscription dies silently every
1.9–4.3h while the socket keeps answering pings. The only liveness signal that exists is the
echo of our own order actions (60/60 placements echoed, p50 0.55s, max 1.36s) — so the watchdog
is: every successful PLACEMENT expects its order id back on the feed within ECHO_DEADLINE_S,
and an expectation that expires means the subscription is dead. Cancels deliberately assert
nothing (their echoes are observed but the rate is unvalidated — no per-cancel timestamps, no
denominator). A cycle that placed nothing asserts nothing — the 3,213s legitimate quiet gap on
a working feed is the case every wall-clock threshold fails.
...
# Verbatim shape from logs/poly_private_ws.jsonl (recorded 2026-07-28) — the field names match
...
def test_overflow_drops_the_OLDEST_and_counts_it():
    ...
    _handle_message [mm-review r1: the first version performed the drop-oldest policy in its
    own body, so deleting the branch under test kept it green]."""
    feed = OrderFeed()
...
# ── the echo watchdog (B2/B5 redesign, 2026-07-31) ──────────────────────────────────────────
...
def test_the_snapshot_watchdog_is_gone():
    """B1 refuted the premise (0 snapshots in 1,419 messages) and B2 showed the watchdog
    false-fires on 82% of subscribes on a HEALTHY feed and reconnect-storms on a dead one.
    Its constants coming back means the design regressed."""
    ...
    assert not hasattr(OrderFeed, "healthy"), (
        "healthy() was the B5 mis-specification (360s wall clock vs a 3,213s legitimate gap); "
        "liveness is check_liveness() now")
...
def test_an_echo_clears_the_expectation_and_the_feed_stays_alive():
    ...
    # mm-review r1: the first cut of this test never aged the expectation, so it was green
...
def test_an_unechoed_placement_past_the_deadline_is_a_death():
    """The single cleanest datum: the WS announced BJRXTE0DWBA7's birth at 02:27:17 and never
    mentioned its death — with the echo watchdog the next cycle's unechoed placement declares
    the subscription dead instead of never."""
    feed = _live_feed()
...
def test_a_quiet_book_asserts_nothing_even_across_the_3213s_real_gap():
    """The longest legitimate quiet gap observed on a WORKING feed was 3,213s — the old
    healthy() (360s) called that dead. No placements => no assertion."""
    feed = _live_feed()
...
def test_an_echo_that_beats_the_rest_response_is_not_a_false_death():
    """The WS delivers at ~81ms median; the REST place response can lose that race. An id seen
...
def test_a_stale_seen_id_does_not_absorb_a_new_expectation():
    ...
    nothing about the subscription NOW. (Historical [mm-review r1]: a bare-membership check
...
def test_the_seen_set_is_pruned_each_check():
    """mm-review r1: unpruned, _seen_ids grows for the run's lifetime and every retained id is
...
def test_an_unechoed_action_on_a_DELIVERING_feed_is_an_anomaly_not_a_death():
    """mm-review r1 (C4): the recorded death mode is TOTAL silence. If messages keep arriving
    ...
    # timestamp, so the guard's INPUT wiring is pinned too
    # _last_msg_mono here left its production write a suite survivor].
...
class TestReconnectLadder:
    """mm-review r1 BLOCKING 2: the storm guard had no test at all. A venue rejecting or
    instantly dropping every fresh subscription must see escalating delays, not a fixed 1s —
    one reconnect per second with errors=0 forever, on an account with a prior Cloudflare
    1015, is the failure both guards exist for."""
